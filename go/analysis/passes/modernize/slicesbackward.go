...
func slicesbackward(pass *analysis.Pass) (any, error) {
	...
	nextLoop:
		for curLoop := range curFile.Preorder((*ast.ForStmt)(nil)) {
			...
			//
			// TextEdits transform the loop header:
			//
			//   for [i := len(s) - 1; i >= 0; i--] { ... [s[i]] ... }
			//        ^^^^^^^^^^^^^^^^^^^^^^^^^^^              ^^^^
			//
			// into (only-index case, otherUses == 0):
			//
			...
			// or (other-uses case):
			//
			//   for [i, v := range slices.Backward(s)] { ... [v] ... }
			//        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^          ^
			sliceStr := astutil.Format(pass.Fset, sliceExpr)
			prefix, importEdits := refactor.AddImport(info, file, "slices", "slices", "Backward", loop.Pos())
